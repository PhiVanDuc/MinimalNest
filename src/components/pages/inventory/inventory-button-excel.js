...
import { addInventoriesExcel } from "@/lib/api/server-action/inventory";

export default function InventoryButtonExcel() {
    const inputRef = useRef(null);

    const [file, setFile] = useState(null);
    const [submitting, setSubmitting] = useState(false);
    const { isDisplay } = useIntentoryExcel();
    ...
    const handleUploadFile = async () => {
        ...
        const uploadFile = await addInventoriesExcel(formData);
    ...
    return (
        <div className={cn(
            "p-[20px] rounded-[10px] bg-white space-y-[20px]",
            isDisplay ? "" : "hidden"
        )}>
            <header className="space-y-[2px]">
                <h2 className="text-[18px] font-semibold">Thêm số lượng với Excel</h2>
