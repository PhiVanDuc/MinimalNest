...
export default function AdminProductEditImage({
    form,
    setEditImages,
    setMainImages,
    setDeletedImages
    ...
    const handleAddImages = (e) => {
        ...
        const existingMainImage = groupImage?.length === 0 ?
        false :
        groupImage.some(image => image?.main);

        const files = Array.from(selectedFiles).map((file, index) => {
            return {
                main: existingMainImage ? false : index === 0,
    ...
    const handleMainImage = (index) => {
        ...
        if (idxInFields !== -1) {
            const currentMain = fields[idxInFields];
            ...
            update(idxInFields, { ...currentMain, main: true });
            move(idxInFields, 0);

            setMainImages(prevState => {
                const indexOldMain = prevState.findIndex(old => (
                    old?.colorId === currentMain?.colorId &&
                    old?.main
                ));

                const copyPrevState = [...prevState];
                copyPrevState.splice(indexOldMain, 1);

                return [...copyPrevState, { ...currentMain, main: true }]
            });
        }
    ...
    const handleRemoveImage = (index) => {
        ...
        if (fields[removeIndexInFields] && typeof fields[removeIndexInFields]?.file === "string") {
            setDeletedImages((prevState) => ([...prevState, fields[removeIndexInFields]?.rootId]));
        }
